...
## Notes
The Dockerfile and `mongod.conf` file are somewhat simplistic so that
additional parameters can be supplied via the `docker run` command to set
individual settings like ports, volumes, etc.

When deploying this container you'll want to have replication go across hosts
and stagger `PRIMARY` and `SECONDARY` nodes across shards to have a redundant
deployment capable of handling failover.